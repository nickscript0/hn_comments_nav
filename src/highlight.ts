...
export class TextHighlight implements Highlight {
    ...
    private _highlightWord(word: string) {
        ...
        Array.from(comments).forEach(e => {
            const text_els = e.getElementsByClassName('c00');
            if (text_els.length === 0) return;

            const original_node = text_els[0];
            ...
            html_element.style.color = 'DarkOrchid';
            html_element.style.fontWeight = 'bold';
